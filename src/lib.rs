...
pub use common::{Config, SniperEvent, MarketData};
pub use accounts::{BondingCurveAccount, TokenInfo};
pub use error::SniperError;

use anyhow::Result;
use common::{StreamClient, Config as StreamConfig};
use tokio::sync::mpsc;
...
impl Sniper {
    ...
    pub async fn start(&mut self) -> Result<(), SniperError> {
        ...
        if self.wallet.is_none() {
            return Err(SniperError::InvalidConfig("No wallet configured".to_string()));
        }
    ...
    async fn handle_event(&mut self, event: SniperEvent) -> Result<(), SniperError> {
        match event {
            SniperEvent::TokenCreated(token_info) => {
                self.handle_token_creation(token_info).await
            }
            SniperEvent::BondingCurveUpdated { bonding_curve, data } => {
            ...
            SniperEvent::BuyTriggered { token_info, market_cap, buy_amount } => {
                self.handle_buy_trigger(token_info, market_cap, buy_amount).await
            }
            _ => Ok(()),
        }
    }
    ...
    async fn handle_token_creation(&mut self, token_info: TokenInfo) -> Result<(), SniperError> {
        ...
        self.tracked_tokens.insert(token_info.mint.to_string(), token_info.clone());
        
        self.check_market_cap(token_info).await
    }

    async fn handle_bonding_curve_update(&mut self, bonding_curve: Pubkey, data: BondingCurveAccount) -> Result<(), SniperError> {
        self.bonding_curve_cache.insert(bonding_curve, data);
        for (_, token_info) in &self.tracked_tokens.clone() {
                        ...
                        Ok(market_cap_usd) => {
                            if market_cap_usd >= self.config.market_cap_threshold_usd {
                                if !self.bought_tokens.contains(&token_info.mint.to_string()) {
                                    ...
                                    info!("INSTANT BUY: {} ${:.0}K", token_info.symbol, market_cap_usd / 1000.0);
    ...
    async fn check_market_cap(&mut self, token_info: TokenInfo) -> Result<(), SniperError> {
        ...
        if let Some(cached_data) = self.bonding_curve_cache.get(&token_info.bonding_curve) {
            ...
            match self.price_fetcher.calculate_market_cap_usd(market_data.current_market_cap_sol).await {
                Ok(market_cap_usd) => {
                    if market_cap_usd >= self.config.market_cap_threshold_usd {
                        info!("CACHED BUY: {} ${:.0}K", token_info.symbol, market_cap_usd / 1000.0);
        ...
        match self.transaction_executor.fetch_bonding_curve_data(&token_info.bonding_curve).await {
            Ok(bonding_curve_data) => {
                ...
                match self.price_fetcher.calculate_market_cap_usd(market_data.current_market_cap_sol).await {
                    Ok(market_cap_usd) => {
                        if market_cap_usd >= self.config.market_cap_threshold_usd {
                            info!("BUY TARGET: {} ${:.0}K", token_info.symbol, market_cap_usd / 1000.0);
                            if !self.bought_tokens.contains(&token_info.mint.to_string()) {
                                if self.test_mode_single_buy && self.has_bought_once {
                                    info!(
                                ...
                                info!("FAST BUY: {} ${:.0}K", token_info.symbol, market_cap_usd / 1000.0);
            ...
            Err(e) => {
                error!("Failed to fetch market data for {}: {}", token_info.symbol, e);
            }
        }
    ...
    async fn handle_market_cap_update(&mut self, _market_data: MarketData) -> Result<(), SniperError> {
        Ok(())
    }
    ...
    async fn handle_buy_trigger(
        ...
        if self.test_mode_single_buy && self.has_bought_once {
            info!("TEST MODE: Skipping buy for {} (already bought once)", token_info.display_name());
            return Ok(());
        }
        ...
        if let Some(wallet) = &self.wallet {
            match self.transaction_executor.execute_buy(wallet, &token_info, buy_amount).await {
                Ok(signature) => {
                    info!(
